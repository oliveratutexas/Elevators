...
from ardiunoCommunicationPackage.msg import SensorStructure
''' new stuff'''
import time, struct
...
intRE = re.compile( r" ?-?[0-9]+ ") #be sure to note the option of a space before and the
  ...
  def run(self):
	...
	while not rospy.is_shutdown():
		if(ser.inWaiting() > 0):
			...
			line = ser.readline()
			rospy.loginfo(line)
			twoInts = intRE.findall(line)
			twoFloats = floatRE.findall(line)
		 	if len(twoInts) == 2:
		 		msgSensor.sonarDistance = int32(twoInts[0]) 
		 		msgSensor.sonarIntensity =int32( twoInts[1]) 
		 	else:
				msgSensor.sonarIntensity = -1
				msgSensor.sonarDistance = -1
		 	if len(twoFloats) == 2:
		 		msgSensor.irDistance = Float32(twoFloats[0])
		 		msgSensor.irDistance2 = Float32(twoFloats[1])
		 	else:
				msgSensor.irDistance = -1.0
				msgSensor.irDistance2 = -1.0
